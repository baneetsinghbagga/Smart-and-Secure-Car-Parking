...
def ratioCheck(area, width, height):
	...
	rmin = 1
	rmax = 7
	#print('area',min,area,max,'rat',ratio)
		...
		if validateRotationAndRatio(min_rect):
			...
			if(isMaxWhite(plate_img)):
				...
				if rect:
					...
					cv2.imshow("Processed",clean_plate)
					#cv2.waitKey(0)
...
def createFrame():
        ...
        count = 0
        while cap.isOpened() and count<21:
                ret,frame = cap.read()
                if ret == True:
...
if True or __name__ == '__main__':
	...
	createFrame()
	img = cv2.imread('./testData/xyz6.jpg')
	dem=(443,332)
	img = cv2.resize(img,dem)
